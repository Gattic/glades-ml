                                                 ...
                                                 size_t n)
{
	const size_t i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i >= n) return;
	const float sa = scaleA ? *scaleA : 1.0f;
	const float sb = scaleB ? *scaleB : 1.0f;
	const float inv = (sa > 0.0f && sb > 0.0f) ? (1.0f / (sa * sb)) : 1.0f;
	const float xv = __bfloat162float(x[i]) * inv;
