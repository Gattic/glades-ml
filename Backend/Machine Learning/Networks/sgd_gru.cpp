	...
	struct ApplyBatch
	{
		...
		bool operator()(int windowsInBatch) const
		{
			...
			if (useAtlas)
			{
				...
				// Output: Why
				if (!atlas::update(tensorGru.O.atlasWhy, &tensorGru.O.Why[0], &tensorGru.O.gWhy[0],
					tensorGru.O.out, tensorGru.O.in, invBatch, lrOut, wd1Out, wd2Out, gradScale,
					ac, rngEngine, logger, "gru.Why"))
				{
					lastStatus = NNetworkStatus(NNetworkStatus::INTERNAL_ERROR, "SGDHelper_GRU: ATLAS Why update entered NaN recovery");
					running = false;
					return false;
				}

				// Output bias: standard SGD
				if (!atlas::updateBias(&tensorGru.O.bias[0], &tensorGru.O.gBias[0], outSize, invBatch, lrOut, gradScale))
				{
				...
				for (int l = 0; l < H; ++l)
				{
					...
					if (!atlas::update(hl.atlasW, &hl.W[0], &hl.gW[0],
						...
						ac, rngEngine, logger, "gru.W"))
					{
						lastStatus = NNetworkStatus(NNetworkStatus::INTERNAL_ERROR, "SGDHelper_GRU: ATLAS W update entered NaN recovery");
						running = false;
						return false;
					}
					...
					if (!atlas::update(hl.atlasU, &hl.U[0], &hl.gU[0],
						...
						ac, rngEngine, logger, "gru.U"))
					{
						lastStatus = NNetworkStatus(NNetworkStatus::INTERNAL_ERROR, "SGDHelper_GRU: ATLAS U update entered NaN recovery");
						running = false;
						return false;
					}
