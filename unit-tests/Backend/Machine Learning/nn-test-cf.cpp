...
void NNCFUnitTestNew()
{
    ...
    shmea::GString netName1 = "xornet";
    glades::NNetwork xorNetwork1;
    shmea::GString netName2 = "xornet";
    glades::NNetwork xorNetwork2;

    if ((xorNetwork1.getEpochs() == 0) && (!xorNetwork1.load(netName1))) {
        printf("[NN] Unable to load \"%s\"", netName1.c_str());
        return;
    }

    if ((xorNetwork2.getEpochs() == 0) && (!xorNetwork2.load(netName1))) {
        printf("[NN] Unable to load \"%s\"", netName2.c_str());
        return;
    }

    // Termination Conditions
    xorNetwork1.terminator.setEpoch(100000);
    xorNetwork1.terminator.setAccuracy(95);

    xorNetwork2.terminator.setEpoch(100000);
    xorNetwork2.terminator.setAccuracy(95);

    std::vector<glades::NNetwork*> networks;
    networks.push_back(&xorNetwork1);
    networks.push_back(&xorNetwork2);

    std::vector<float> averageAccuracies; 
    unsigned int foldsNum = 2;
    glades::MetaNetwork* cMetaNetwork = glades::crossValidate(networks, inputTable, glades::DataInput::CSV, averageAccuracies, foldsNum);
...
///////////////////////Test Iris/////////////////////////
    ...
    foldsNum = 10;
    cMetaNetwork = glades::crossValidate(irisNetworks , *shuffled_inputIrisTable, glades::DataInput::CSV, averageAccuracies, foldsNum);
