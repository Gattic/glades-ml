...
SKIP_BUILD=0
SCALES_CSV="1.0,0.75"
SCOPES_CSV="all,large-only,late-head,late-head-large"
BENCHMARKS_CSV="token-lm-document,token-lm-corpus-large"
EPOCHS_CSV="1,2,3,4"
TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
OUT_DIR_DEFAULT="$ROOT_DIR/artifacts/echo_scope_sweep_${TIMESTAMP}"
...
Runs a narrow ECHO scope GPU sweep against AdamW and saves raw logs plus TSV summaries.

Defaults:
  benchmarks: token-lm-document,token-lm-corpus-large
  scales:     1.0,0.75
  scopes:     all,large-only,late-head,late-head-large
  epochs:     1,2,3,4
...
Options:
  ...
  --scopes CSV          Comma-separated ECHO scope list
...
cat > "$SUMMARY_TSV" <<'EOF'
benchmark	optimizer	echo_scale	echo_scope	epochs	train_s_mean	train_s_pm	tok_s_mean	tok_s_pm	train_nll_mean	train_nll_pm	test_nll_mean	test_nll_pm	status	logfile
EOF

cat > "$ACCEPT_TSV" <<'EOF'
benchmark	optimizer	echo_scale	echo_scope	train_s_mean	train_s_pm	tok_s_mean	tok_s_pm	train_nll_mean	train_nll_pm	test_nll_mean	test_nll_pm	status	logfile
EOF
...
append_epoch_summary() {
  ...
  local epochs="$5"
  local logfile="$6"
  awk -v benchmark="$benchmark" -v optimizer="$optimizer" -v scale="$scale" -v scope="$scope" -v epochs="$epochs" -v logfile="$logfile" '
    /^(AdamW|ATLAS-ECHO)[[:space:]]/ && NF >= 20 {
      printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n",
             benchmark, optimizer, scale, scope, epochs,
...
append_acceptance_summary() {
  ...
  local scope="$4"
  local logfile="$5"
  awk -v benchmark="$benchmark" -v optimizer="$optimizer" -v scale="$scale" -v scope="$scope" -v logfile="$logfile" '
    /^(AdamW|ATLAS-ECHO)[[:space:]]/ && NF >= 20 {
      printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n",
             benchmark, optimizer, scale, scope,
             $2, $4, $5, $7, $8, $10, $14, $16, $20, logfile;
      exit 0;
    }
  ' "$logfile" >> "$ACCEPT_TSV"
...
run_variant_epoch() {
  ...
  append_epoch_summary "$benchmark" "$optimizer" "$scale" "$scope" "$epochs" "$OUT_DIR/raw/${name}.log"
}
...
run_variant_accept() {
  ...
  run_capture "$name" \
    ...
    --gpu-enable 1 \
    --gpu-device "$GPU_DEVICE"
  append_acceptance_summary "$benchmark" "$optimizer" "$scale" "$scope" "$OUT_DIR/raw/${name}.log"
}
  ...
  for epochs in "${EPOCHS[@]}"; do
    ...
    for scale in "${SCALES[@]}"; do
      for scope in "${SCOPES[@]}"; do
        ...
        tag_scope="$(scope_tag "$scope")"
        run_variant_epoch "sweep_${benchmark}_echo_s${tag_scale}_${tag_scope}_e${epochs}" \
          "$benchmark" "$epochs" "echo" "$scale" "$scope" \
          --variant echo \
  ...
  for benchmark in "${BENCHMARKS[@]}"; do
    ...
    for scale in "${SCALES[@]}"; do
      for scope in "${SCOPES[@]}"; do
        tag_scale="$(scale_tag "$scale")"
        tag_scope="$(scope_tag "$scope")"
        run_variant_accept "accept_${benchmark}_echo_s${tag_scale}_${tag_scope}" \
          "$benchmark" "echo" "$scale" "$scope" \
          --variant echo \
          --atlas-echo-geometry-scale "$scale" \
          --atlas-echo-scope "$scope"
      done
...
- epoch_sweep_summary.tsv: one row per benchmark/optimizer/scale/scope/epoch point
