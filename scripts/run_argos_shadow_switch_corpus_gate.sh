...
ARGOS_HEAD=0.35
ARGOS_LATE=0.00
ARGOS_SHADOW_FRACTIONS_CSV="0.50,0.67,0.75"

TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
OUT_DIR_DEFAULT="$ROOT_DIR/artifacts/argos_shadow_switch_corpus_gate_${TIMESTAMP}"
...
Runs a corpus-large GPU gate comparing:
  - AdamW
  - ATLAS-ARGOS
  - ATLAS-ARGOS-SHADOW-SWITCH across state-only fractions

Default shadow fractions:
  ${ARGOS_SHADOW_FRACTIONS_CSV}
...
Outputs:
  ...
  - shadow_acceptance_rank_by_nll.tsv
  - shadow_epoch4_rank_by_nll.tsv
  - raw/*.log
...
Options:
  ...
  --atlas-argos-shadow-fractions CSV   Comma-separated state-only fractions in [0,1] (default: ${ARGOS_SHADOW_FRACTIONS_CSV})
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --skip-acceptance)
      ...
      shift
      ;;
    --atlas-argos-shadow-fractions)
      ARGOS_SHADOW_FRACTIONS_CSV="$2"
      shift 2
      ;;
...
IFS=',' read -r -a ARGOS_SHADOW_FRACTIONS <<< "$ARGOS_SHADOW_FRACTIONS_CSV"
if [[ "${#ARGOS_SHADOW_FRACTIONS[@]}" -eq 0 ]]; then
  echo "empty --atlas-argos-shadow-fractions list" >&2
  exit 2
fi
for fraction in "${ARGOS_SHADOW_FRACTIONS[@]}"; do
  if [[ ! "$fraction" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
    echo "invalid ARGOS shadow fraction: $fraction" >&2
    exit 2
  fi
  if ! awk -v x="$fraction" 'BEGIN { exit !(x >= 0.0 && x <= 1.0) }'; then
    echo "ARGOS shadow fraction out of range [0,1]: $fraction" >&2
    exit 2
  fi
done
...
SHADOW_ACCEPT_RANK_TSV="$OUT_DIR/shadow_acceptance_rank_by_nll.tsv"
SHADOW_E4_RANK_TSV="$OUT_DIR/shadow_epoch4_rank_by_nll.tsv"
...
run_capture_optional() {
  ...
  shift
  local logfile="$OUT_DIR/raw/${name}.log"
  echo "[run] $name (non-fatal)" | tee -a "$RUN_LOG" >&2
  log_cmd "$@"
  set +e
  {
    printf '$'
  ...
  set -e
  if [[ "$status" -ne 0 ]]; then
    echo "[warn] $name failed with exit code $status; continuing" | tee -a "$RUN_LOG" >&2
  fi
  return 0
}
...
append_epoch_summary() {
  ...
  awk -v benchmark="$benchmark" -v optimizer="$optimizer" -v epochs="$epochs" -v logfile="$logfile" '
    /^(AdamW|ATLAS-ARGOS|ATLAS-ARGOS-SHADOW-SWITCH)[[:space:]]/ && NF >= 20 {
...
generate_epoch4_rank() {
  ...
  {
    printf "benchmark\trank\toptimizer\tepochs\ttrain_s_mean\ttest_nll_mean\tlogfile\n"
    awk -F '\t' '$1 == "'"$BENCHMARK"'" && $2 ~ /^argos_shadow_switch_f/ && $3 == "4"' "$SUMMARY_TSV" \
...
generate_acceptance_rank() {
  ...
  {
    printf "benchmark\trank\toptimizer\ttrain_s_mean\ttest_nll_mean\tlogfile\n"
    awk -F '\t' '$1 == "'"$BENCHMARK"'" && $2 ~ /^argos_shadow_switch_f/' "$ACCEPT_TSV" \
      | sort -t $'\t' -k9,9g -k3,3g \
      | awk -F '\t' '
          BEGIN { OFS = "\t"; rank = 0; }
          {
...
run_capture_optional 06_smoke_argos_shadow_switch_gpu \
  "$BIN" atlas-alt-bench \
  --mode "$BENCHMARK" \
  --token-epochs 1 \
  --repeats 1 \
  "${SHADOW_ARGS_BASE[@]}" \
  --atlas-argos-switch-fraction "${ARGOS_SHADOW_FRACTIONS[0]}" \
  --gpu-enable 1 \
  --gpu-device "$GPU_DEVICE"
...
for epochs in "${EPOCHS[@]}"; do
  ...
  for fraction in "${ARGOS_SHADOW_FRACTIONS[@]}"; do
    label="argos_shadow_switch_f$(fraction_label "$fraction")"
...
Files:
  ...
  shadow_acceptance_rank_by_nll.tsv ARGOS shadow-switch acceptance ranking
  shadow_epoch4_rank_by_nll.tsv     ARGOS shadow-switch epoch-4 ranking
...
Shadow-switch meaning:
  ARGOS state updates from step 1, but the applied deviation from the exact
  AdamW backbone is clamped to zero until the configured fraction of training.
EOF
...
if [[ "$RUN_ACCEPTANCE" -eq 1 ]]; then
  ...
  echo "Shadow acceptance ranking: $SHADOW_ACCEPT_RANK_TSV"
fi
echo "Epoch-4 ranking: $E4_RANK_TSV"
echo "Shadow epoch-4 ranking: $SHADOW_E4_RANK_TSV"
