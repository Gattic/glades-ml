...
OUT_DIR_DEFAULT="$ROOT_DIR/artifacts/matra_gpu_gate_${TIMESTAMP}"
...
Runs the MATRA-vs-AdamW GPU benchmark gate and saves raw logs plus TSV summaries.
...
run_capture 06_smoke_matra_gpu \
  "$BIN" atlas-alt-bench \
  --mode token-lm \
  --token-epochs 1 \
  --repeats 1 \
...
Primary decision rule:
  MATRA only survives as a practical branch if it improves held-out TestNLL
