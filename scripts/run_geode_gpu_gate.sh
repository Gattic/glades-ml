...
OUT_DIR_DEFAULT="$ROOT_DIR/artifacts/geode_gpu_gate_${TIMESTAMP}"
...
Runs the GEODE-vs-AdamW GPU benchmark gate and saves raw logs plus a TSV summary.

Options:
  --gpu-device N      CUDA device id to request (default: 0)
  --rank N            GEODE rank to use (default: 8)
...
run_capture 04_smoke_geode_gpu \
  "$BIN" atlas-alt-bench \
  --mode token-lm \
  --token-epochs 1 \
  --repeats 1 \
...
for benchmark in token-lm-document token-lm-corpus-large; do
  for epochs in "${EPOCHS[@]}"; do
    adam_log="sweep_${benchmark}_adamw_e${epochs}"
    geode_log="sweep_${benchmark}_geode_e${epochs}"

    run_capture "$adam_log" \
      "$BIN" atlas-alt-bench \
      --mode "$benchmark" \
      --token-epochs "$epochs" \
      --repeats "$REPEATS" \
      ...
      --gpu-enable 1 \
      --gpu-device "$GPU_DEVICE"
    append_epoch_summary "$benchmark" "adamw" "$epochs" "$OUT_DIR/raw/${adam_log}.log"

    run_capture "$geode_log" \
      "$BIN" atlas-alt-bench \
      --mode "$benchmark" \
      --token-epochs "$epochs" \
      --repeats "$REPEATS" \
      ...
      --gpu-enable 1 \
      --gpu-device "$GPU_DEVICE"
    append_epoch_summary "$benchmark" "geode" "$epochs" "$OUT_DIR/raw/${geode_log}.log"
  done
done

if [[ "$RUN_ACCEPTANCE" -eq 1 ]]; then
  for benchmark in token-lm-document token-lm-corpus-large; do
    adam_log="accept_${benchmark}_adamw"
    geode_log="accept_${benchmark}_geode"
    ...
    run_capture "$adam_log" \
      ...
      --gpu-enable 1 \
      --gpu-device "$GPU_DEVICE"
    append_acceptance_summary "$benchmark" "adamw" "$OUT_DIR/raw/${adam_log}.log"
    ...
    run_capture "$geode_log" \
      ...
      --gpu-enable 1 \
      --gpu-device "$GPU_DEVICE"
    append_acceptance_summary "$benchmark" "geode" "$OUT_DIR/raw/${geode_log}.log"
  done
fi
