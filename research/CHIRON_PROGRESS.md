...
| 19 | **IBGRAD — Information-Bottleneck Gradient Subspace (PHASES 1-2 SHIPPED)** | gradient subspace rank (Adam state + backward compute) | **20× Adam state + 20× backward GEMM** at r=0.05·N per block; composes multiplicatively with OVFG/MFIO/DFA/GFIB; **compound TRCD × LCP × IBGRAD = 282× theoretical** (56× realistic at 20% realization) | 5 primitives: `ibgrad_init_projection` / `_project` / `_unproject` / `_oja_rank1_update` / `_qr_reorthogonalize` (cuSOLVER); 2 parity tests: init var 0.99× expected, proj/unproj 1e-7, Oja bit-exact, Oja sanity ‖Pᵀg‖² grows 1.89× after one step, QR ‖QᵀQ − I‖ = 7e-7, column-space preserved 1e-4; design `PARADIGM_SHIFT_19_{SELECTION,CANDIDATE_A/B/C}.md` |
