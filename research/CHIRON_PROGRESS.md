...
| 19 | **IBGRAD — Information-Bottleneck Gradient Subspace (PHASES 1-3 SHIPPED, F2 confirmed)** | gradient subspace rank (Adam state + backward compute) | **20× Adam state + 20× backward GEMM** at r=0.05·N per block; composes multiplicatively with OVFG/MFIO/DFA/GFIB; **compound TRCD × LCP × IBGRAD = 282× theoretical** | 5 primitives + 3 tests: parity (proj/unproj/Oja), QR (‖QᵀQ − I‖=7e-7), E2E subspace Adam (1.46× loss reduction plateau — **empirical confirmation of F2 failure mode** from design doc: subspace captures ~33% of loss-reducible direction, Phase 4 audit needed to close to dense Adam); design `PARADIGM_SHIFT_19_{SELECTION,CANDIDATE_A/B/C}.md` |
