...
| 19 | **IBGRAD — Information-Bottleneck Gradient Subspace (PHASE 1 SHIPPED)** | gradient subspace rank (Adam state + backward compute) | **20× Adam state + 20× backward GEMM** at r=0.05·N per block; composes multiplicatively with OVFG/MFIO/DFA/GFIB; **compound TRCD × LCP × IBGRAD = 282× theoretical** (56× realistic at 20% realization) | 4 primitives: `ibgrad_init_projection` / `_project` / `_unproject` / `_oja_rank1_update`; parity test passes (init var 0.99× expected, proj/unproj at 1e-7, Oja bit-exact, ‖Pᵀg‖² grows 1.89× after one step); design `PARADIGM_SHIFT_19_{SELECTION,CANDIDATE_A/B/C}.md` |
