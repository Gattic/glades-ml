...
| 11 | **MFIO — Moment-Free Implicit Optimizer (PHASE 1 SHIPPED)** | optimizer state | **ZERO per-parameter state** (eliminates m, v); linear regression: 80.9% of Adam's log-descent rate; **nonlinear 2-layer MLP: 102% of Adam (MFIO wins)** | `mfio_compute_sigma` + `mfio_update` primitives; σ = 1/(√(ŝ·β)+ε) from activation-norm reduction; 3 tests incl. head-to-head vs Adam; Phase 2 trainer wire-in pending |
