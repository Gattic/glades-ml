...
| 19 | **IBGRAD — Information-Bottleneck Gradient Subspace (PHASES 1-4 SHIPPED, VALIDATED)** | gradient subspace rank (Adam state + backward compute) | **20× Adam state + 20× backward GEMM** at r=0.05·N per block; composes multiplicatively with OVFG/MFIO/DFA/GFIB; **compound TRCD × LCP × IBGRAD = 282× theoretical**; **E2E 241.63× loss reduction** with Phase 4 audit | 5 primitives + 3 tests all pass: parity (proj/unproj/Oja), QR (‖QᵀQ − I‖=7e-7), E2E subspace Adam 241.63× (F2 audit closed the plateau: 1.46× → 241.63×, 165× multiplier from the "safeguard" mechanism); design `PARADIGM_SHIFT_19_{SELECTION,CANDIDATE_A/B/C}.md` |
