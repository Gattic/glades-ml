...
- Large (≥1B): `--face-beta-row 0.98` (validated at 955M × 1000: -0.33 nat iter 111; 1.19B × 1000: -0.32 nat iter 114; 1.41B × 1000: iter 117 pending dense)
...
| 13 | **TRCD — Token-Routed Conditional Depth (PHASES 1-2 SHIPPED)** | per-token forward/backward FLOPs | **3× FLOP reduction** at d̄=L/3; first paradigm shift to attack data-side per-token variable compute; composes MULTIPLICATIVELY with all shifts 1-12; E2E **loss drops 187× with budget tracking at 7.8% err** | `trcd_route_logits/_backward` + `trcd_gumbel_gate` + `trcd_apply_gate/_backward` + `trcd_lambda_pi_update` primitives; 6 tests: 5 parity (route fwd+bwd 5e-7/2e-7, gate eval+train, apply fwd+bwd <1e-6, λ-PI converges 35 steps) + E2E convergence (2-layer ReLU MLP, 300 Adam steps, 187× loss drop, d̄ tracks 7.8% err); design `PARADIGM_SHIFT_13_{SELECTION,CANDIDATE_A/B/C}.md` |
| 16 | **LCP — Lattice Compute Pooling (PHASES 1-2 SHIPPED, VALIDATED E2E)** | per-token forward compute + activation memory | **4.7× per-layer standalone; 24× compound with TRCD** (row × column factoring of L×T compute matrix); O(M·d) activation additive with CHIRON; **E2E 13.72× MSE reduction** (detail network closes cluster-pool gap in 200 Adam steps) | 7 primitives + 4 tests all pass (gather-scatter roundtrip, delta parity, E2E detail correction at 13.72×, routing bench 0.162 ms/cycle at pile_large); `--lcp-preview` flag shows projected speedups (6.14× at M=T/16); design `PARADIGM_SHIFT_16_{SELECTION,CANDIDATE_A/B/C}.md` |
| 19 | **IBGRAD — Information-Bottleneck Gradient Subspace (PHASES 1-5 SHIPPED, TRAINER WIRE-IN COMPLETE)** | gradient subspace rank (Adam state + backward compute) | **20× Adam state + 20× backward GEMM** at r=0.05·N per block; composes multiplicatively with OVFG/MFIO/DFA/GFIB; **compound TRCD × LCP × IBGRAD = 282× theoretical**; **E2E 241.63× loss reduction** with Phase 4 audit; **chiron_train --ibgrad-rank R active on Wo matrices** | 8 primitives + 6 tests all pass; Phases 1-5 complete: 5A primitives (apply_update, captured_fraction), 5B state alloc (per-layer Wo_P, m_sub, v_sub), 5C adam_step wire-in (project→scratch Adam→unproject→apply+Oja+audit); chiron_train smoke test 75,925 tok/s with IBGRAD on 4-layer config, loss decreasing normally |
...
| +  | **Local-attn wire-in to main sgd_transformer** | attention compute on production trainer | `--local-attn W` in `glades_pile_train` at pile_large: 50,890 tok/s at seq_len=2048 W=256, clean training through 21 opt steps, loss 10.58 stable | commits: `18b1d1330 feat(local-attn)` + `ad71953 feat(local-attn)` (glades-trainer) |
