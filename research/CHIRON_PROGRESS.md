...
| 13 | **TRCD — Token-Routed Conditional Depth (PHASE 1 SHIPPED)** | per-token forward/backward FLOPs | **3× FLOP reduction** at d̄=L/3; first paradigm shift to attack data-side per-token variable compute; composes MULTIPLICATIVELY with all shifts 1-12 | `trcd_route_logits/_backward` + `trcd_gumbel_gate` + `trcd_apply_gate/_backward` + `trcd_lambda_pi_update` primitives; 5 parity tests (route fwd+bwd at 5e-7/2e-7, gate eval+train, apply fwd+bwd at <1e-6, λ-PI converges in 35 steps to target d̄=12); design `PARADIGM_SHIFT_13_{SELECTION,CANDIDATE_A/B/C}.md` |
