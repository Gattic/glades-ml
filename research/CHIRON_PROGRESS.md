...
| 16 | **LCP — Lattice Compute Pooling (PHASES 1-2 SHIPPED)** | per-token forward compute + activation memory | **4.7× per-layer standalone; 24× compound with TRCD** (row × column factoring of L×T compute matrix); O(M·d) activation additive with CHIRON | 7 primitives: `lcp_lsh_init_matrix` / `_project` / `_bucket_first_index` / `_gather` / `_scatter` / `_scatter_backward` / `_compute_delta` / `_compute_delta_backward`; 2 parity tests all pass: hash-consistent 32/32, scatter cluster-invariant (max diff 0), deterministic, delta fwd/bwd at 0 / 2.4e-7; design `PARADIGM_SHIFT_16_{SELECTION,CANDIDATE_A/B/C}.md`; scouting `PARADIGM_SHIFT_19_AXIS_NOTES.md` for next axis (R1 precision heterogeneity or R3 predictive forward) |
