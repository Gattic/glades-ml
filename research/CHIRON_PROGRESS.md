...
| 16 | **LCP — Lattice Compute Pooling (PHASE 1 SHIPPED)** | per-token forward compute + activation memory | **4.7× per-layer standalone; 24× compound with TRCD** (row × column factoring of L×T compute matrix); O(M·d) activation additive with CHIRON | `lcp_lsh_init_matrix` + `lcp_lsh_project` + `lcp_bucket_first_index` + `lcp_gather`/`_scatter`/`_scatter_backward` primitives; 1 parity test: identical 4-token groups hash-consistent 32/32, scatter cluster-invariant (max diff 0), deterministic; design `PARADIGM_SHIFT_16_{SELECTION,CANDIDATE_A/B/C}.md` — LCP selected over GFIB (break-even) and SGS (1.9×) |
