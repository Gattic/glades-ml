...
**Status:** PARTIALLY FALSIFIED (iter 13, 14, 2026-05-15). Iter 13 falsified the Σ-based defect (Pearson r = −0.187 vs Phase 8b NLL). Iter 14 tested Candidate 1 (U-frame defect ε^U_i = ‖U_i^T U_{i-1} − I_r‖_F) — correlation jumped to +0.475 (close to 0.5 threshold) but magnitude spread stayed at ~1%, ratio test still fails. See `DSA_PROBE_O_FRAME_RESULT.md` for the iter-14 comparative finding. The paradigm's infrastructure (both Σ and U-frame defect kernels + parity test + trainer wiring) is shipped; the gate-driver formula remains weak at 200-step training. Iter-15 candidates: longer training (2000 steps) to sharpen signal, or Candidate 2 (residual-stream defect ε^q).
