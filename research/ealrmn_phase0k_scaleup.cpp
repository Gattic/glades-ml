// EALRMN-v1 Phase-0k prototype — SCALE-UP of Phase-0g.
//
// Tests whether the modest mechanism gains observed in Phase-0a-0g compound
// at moderate scale: m=64 (2×), d_emb=32 (2×), default T=256 (4×), 5000 steps.
// Architecture and training logic identical to Phase-0g — ONLY constants and
// defaults change. Pre-registered prediction: if scale unlocks the predicted
// compounding, EALRMN attmem at T≥256 should show a substantial lead over RNN
// (>0.10 acc). If not, the writeup's Option D recommendation is reinforced.
//
// Original Phase-0g comments retained below for traceability:
// ---
// Phase-0g prototype — attention-based memory read.
//
...
// Phase-0k scale-up: 2× embedding dim, 2× operator dim.
static const int kEmbDim       = 32;
...
int main(int argc, char** argv) {
    ...
    int seed = 42; int steps = 5000; int print_every = 500; int T = 256;
    double lr = 0.003; bool aux_class = false; bool aux_gate = false;
    ...
    tr.lr = lr;
    std::printf("# Phase-0k SCALE-UP  model=%s seed=%d steps=%d T=%d lr=%.4f d_emb=%d m=%d aux_class=%d aux_gate=%d\n",
                model_str.c_str(), seed, steps, T, lr, kEmbDim, kM, aux_class?1:0, aux_gate?1:0);
